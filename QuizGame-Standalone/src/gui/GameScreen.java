...
public class GameScreen extends JPanel {
        ...
        public GameScreen() {
                ...
                // We'll wrap the question label in a scroll pane so long questions can scroll
                questionLabel.setBounds(0, 0, 1200, 520);

                questionScroll = new JScrollPane(questionLabel);
                questionScroll.setBounds(200, 250, 1200, 520);
                questionScroll.setBorder(null);
                // Make the viewport transparent so background shows through (label is
                // non-opaque)
        ...
        private void displayQuestion(Question question) {
                if (question != null) {
                        // Build a single HTML content that contains the question and the choices
                        String[] opts = question.getOptions();
                        currentChoiceTexts = opts; // save raw text

                        questionLabel.setText(formatQuestionWithChoices(question.getQuestionText(), opts));

                        // Hide the separate choice text labels (we render choices inside the question
                        // pane)
                        choiceAText.setVisible(false);
                        choiceBText.setVisible(false);
                        choiceCText.setVisible(false);
                        choiceDText.setVisible(false);
        ...
        private String formatQuestionText(String text) {
                ...
                // Return the inner HTML body (without <html> wrapper). The caller will wrap
                // the full content when appending choices.
                return "<div style='font-family:Arial; font-size:22px; color:#333333; text-align:center; margin-bottom:15px;'>"
                                + "QUESTION:"
                                + "</div>"
        ...
        /**
         * Build full HTML including question, optional code box and the four choices.
         * Choices will appear immediately after the question so they are visually
         * close.
         */
        private String formatQuestionWithChoices(String text, String[] opts) {
                // Start with the question body (no outer html tag)
                String body = formatQuestionText(text);

                // Build choices HTML. Convert literal '\\n' to <br> for line breaks inside
                // choices.
                StringBuilder sb = new StringBuilder();
                sb.append("<div style='width:100%; margin-top:20px;'>");

                for (int i = 0; i < opts.length && i < 4; i++) {
                        String prefix = getChoicePrefix(i);
                        String opt = opts[i] == null ? "" : opts[i];
                        // convert escaped newlines to HTML breaks
                        String optHtml = opt.replace("\\n", "<br>");

                        sb.append("<div style='font-family:Arial; font-size:20px; color:#333333; text-align:left; margin:12px 8% 12px 8%; width:84%;'>");
                        sb.append("<span style='font-weight:bold; margin-right:8px;'>" + prefix + "</span>");
                        sb.append("<span style='font-family:Consolas, monospace; font-size:18px; color:#333333;'>"
                                        + optHtml + "</span>");
                        sb.append("</div>");
                }
                ...
                // Wrap everything in html tag and return
                return "<html>" + body + sb.toString() + "</html>";
        }
        ...
        private void highlightChoices(int selectedIndex, int correctIndex) {
                // If we're rendering choices inside the question pane (we hid the separate
                // labels),
                // rebuild the question HTML with colored choices.
                if (choiceAText != null && !choiceAText.isVisible()) {
                        questionLabel.setText(formatQuestionWithColoredChoices(currentQuestion.getQuestionText(),
                                        currentChoiceTexts, selectedIndex, correctIndex));
                        return;
                }
         ...
         * Build full HTML including question, optional code box and the four choices
         * with
         * colored highlights for selected and correct choices.
         */
        private String formatQuestionWithColoredChoices(String text, String[] opts, int selectedIndex,
                        int correctIndex) {
                String body = formatQuestionText(text);
                ...
                for (int i = 0; i < opts.length && i < 4; i++) {
                        ...
                        sb.append("<div style='font-family:Arial; font-size:20px; color:" + color
                                        + "; text-align:left; margin:12px 8% 12px 8%; width:84%;'>");
                        sb.append("<span style='font-weight:bold; margin-right:8px;'>" + prefix + "</span>");
                        sb.append("<span style='font-family:Consolas, monospace; font-size:18px; color:" + color
                                        + ";'>" + optHtml + "</span>");
                        sb.append("</div>");
                }
        ...
        private void useAutoDebug() {
                ...
                // If we're rendering choices inside the question pane, update that HTML
                if (choiceAText != null && !choiceAText.isVisible()) {
                        questionLabel.setText(formatQuestionWithColoredChoices(currentQuestion.getQuestionText(),
                                        currentChoiceTexts, -1, correctIndex));
                } else {
                        JLabel[] labels = { choiceAText, choiceBText, choiceCText, choiceDText };
                        for (int i = 0; i < labels.length; i++) {
                                if (i == correctIndex)
                                        labels[i].setText(
                                                        formatChoiceText(getChoicePrefix(i), currentChoiceTexts[i],
                                                                        "#00AA00"));
                        }
                }
